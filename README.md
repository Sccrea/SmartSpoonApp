...
应用默认连的是 **`https://sccrea64.cc.cd:16384`**（`State.DEFAULT_SERVER`，见 `Data.kt`）；
启动时会探测它以及用户保存过的地址，第一个 `/api/health` 通的就被采用，连上后读取菜品信息。
也可以在 **设置 → 杂项 → 服务器地址** 里「修改地址」或「重新读取菜品信息」。

> 换服务器只改 `Data.kt` 里那一个常量即可（`FALLBACK_SERVERS` 是同一批候选地址）。
